 ...
 * para uso no comercial en <https://boardgamegeek.com/using_the_xml_api>; el token
 * viaja en la cabecera `Authorization` y lo pone quien llama, que es lo que permite que
 * este fichero no sepa de dónde sale.
 *
 * BGG pide ir despacio (~1 petición cada 2 s) y responde `202` mientras prepara la
 * respuesta, así que hay que reintentar en vez de darlo por vacío. Ese ritmo es el techo
 * del token ENTERO, no de cada usuario, y es la razón de que la app nunca hable con BGG
 * directamente: lo hacen los scripts en diferido y la función `resolve-game`, que además
 * apunta cada consulta en `catalog_misses` para no repetirla.
 *
 * Vive aquí, y no en `scripts/lib/`, porque lo usan dos tiempos de ejecución distintos:
 * Node (`npm run ingest:bgg`) y Deno (la función `resolve-game`). Por eso no importa
 * nada: ni `node:fs`, ni `Deno.env`, ni el motor de la app. Solo `fetch`, que en los dos
 * es el mismo. Quien necesita el token lo lee en su casa —`readToken()` en
 * `scripts/lib/bgg-api.ts`, `Deno.env` en la función— y lo pasa por parámetro.
 */
