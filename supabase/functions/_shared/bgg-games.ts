 ...
 *
 * Está aparte del script de ingesta porque es lo único de la traducción BGG → catálogo
 * que se puede comprobar sin red: decenas de miles de filas se escriben con estas
 * reglas y nadie las va a mirar una a una, así que se testean en
 * `scripts/lib/bgg-games.test.ts`.
 *
 ...
 *
 * Vive en `supabase/functions/_shared/` porque lo usan los dos caminos que traen juegos
 * de BGG y corren en tiempos de ejecución distintos: `npm run ingest:bgg` en Node y la
 * función `resolve-game` en Deno. Un juego tiene que entrar igual por los dos, así que
 * la traducción se escribe una sola vez. Por eso lo único que importa es el motor del
 * catálogo, con la extensión puesta: Deno la exige y a Vite le da igual.
 */
import { expandCatalogSeedRow, type SheetId } from '../../../src/games/catalog.ts'
import type { BggThing } from './bgg-api.ts'
import type { GameDefinition, GameDifficulty } from '../../../src/games/types.ts'
